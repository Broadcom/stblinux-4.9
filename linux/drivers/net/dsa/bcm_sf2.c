...
static unsigned bcm_sf2_num_active_ports(struct dsa_switch *ds)
{
...
static int bcm_sf2_sw_resume(struct dsa_switch *ds)
{
	...
	/* Bump up the switch core clock to let the CFP RAMs latch
	 * data in correctly
	 */
	if (priv->clk_mdiv)
		clk_set_rate(priv->clk_mdiv, 250000000);
...
static int bcm_sf2_sw_probe(struct platform_device *pdev)
{
	...
	ds->ops->get_rxnfc = bcm_sf2_get_rxnfc;
	ds->ops->set_rxnfc = bcm_sf2_set_rxnfc;
